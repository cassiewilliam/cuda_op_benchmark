...
void mha_nofuse_kernel(cudaStream_t stream,
                       ...
                       const __half scaler)
{
    const int buffer_size = batch_size * seq_len * head_num * size_per_head;
    __half* q_buf_ = buffer;
    __half* k_buf_ = q_buf_ + buffer_size;
    __half* v_buf_ = k_buf_ + buffer_size;
    __half* transpose_dst_ = v_buf_ + buffer_size;
    __half* qk_buf_ = transpose_dst_ + buffer_size;
    ...
    add_QKV_bias<__half><<<grid, block, 0, stream>>>(Q, bias_Q, K, bias_K, V, bias_V, q_buf_, k_buf_, 
    ...
    if (0)
    {
        std::vector<half> temp_q_buffer;
        temp_q_buffer.resize(buffer_size);

        std::vector<half> temp_k_buffer;
        temp_k_buffer.resize(buffer_size);

        cudaMemcpy(reinterpret_cast<half*>(temp_q_buffer.data()), reinterpret_cast<half*>(q_buf_), sizeof(half) * buffer_size, cudaMemcpyDeviceToHost);
        cudaMemcpy(reinterpret_cast<half*>(temp_k_buffer.data()), reinterpret_cast<half*>(k_buf_), sizeof(half) * buffer_size, cudaMemcpyDeviceToHost);

        for (int i = 0; i < 2000; ++i)
        {
            printf("gpu: %d, %.2f, %.2f\n", i, (float)(((half *)temp_q_buffer.data())[i]), (float)(((half *)temp_k_buffer.data())[i]));
        }
    }
    ...
    check_cuda_error(cublasGemmStridedBatchedEx(cublas_handle,
      ...
      &alpha,
      k_buf_, CUDA_R_16F, size_per_head, seq_len * size_per_head,
      q_buf_, CUDA_R_16F, size_per_head, seq_len * size_per_head,
      &beta,
      qk_buf_, CUDA_R_16F, seq_len, seq_len * seq_len,
      batch_size * head_num,
      CUDA_R_16F,
    ...
    if (0)
    {
        const int qk_buffer_size = batch_size * seq_len * seq_len;
        std::vector<half> temp_qk_buffer;
        temp_qk_buffer.resize(qk_buffer_size);

        cudaMemcpy(reinterpret_cast<half*>(temp_qk_buffer.data()), reinterpret_cast<half*>(qk_buf_), sizeof(half) * qk_buffer_size, cudaMemcpyDeviceToHost);

        for (int i = 0; i < 2000; ++i)
        {
            printf("gpu: %d, %.2f\n", i, (float)(((half *)temp_qk_buffer.data())[i]));
        }
    }
    ...
    if(batch_size * head_num <= 120)
    {
      ...
      softmax_kernel_v2<__half><<<grid, block, 0, stream>>>(qk_buf_, attr_mask, batch_size, head_num, seq_len, scaler); 
    }
    else
    {
      grid.x = batch_size * head_num;
      softmax_kernel<__half><<<grid, block, 0, stream>>>(qk_buf_, attr_mask, batch_size, head_num, seq_len, scaler); 
    }
    ...
    check_cuda_error(cublasGemmStridedBatchedEx(cublas_handle,
                      ...
                      &alpha,
                      v_buf_, CUDA_R_16F, size_per_head, seq_len * size_per_head,
                      qk_buf_, CUDA_R_16F, seq_len, seq_len * seq_len,
                      &beta,
                      transpose_dst_, CUDA_R_16F, size_per_head, seq_len * size_per_head,
                      batch_size * head_num,
                      CUDA_R_16F,
                      static_cast<cublasGemmAlgo_t>(99)));
      ...
      transpose<__half><<<grid, block, 0, stream>>>(transpose_dst_, dst, 
