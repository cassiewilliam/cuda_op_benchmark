...
#define FT_LOG(level, ...)                                                                                             \
    do {                                                                                                               \
        if (ft::Logger::getLogger().getLevel() <= level) {                                              \
            ft::Logger::getLogger().log(level, __VA_ARGS__);                                            \
        }                                                                                                              \
    } while (0)

#define FT_LOG_TRACE(...) FT_LOG(ft::Logger::TRACE, __VA_ARGS__)
#define FT_LOG_DEBUG(...) FT_LOG(ft::Logger::DEBUG, __VA_ARGS__)
#define FT_LOG_INFO(...) FT_LOG(ft::Logger::INFO, __VA_ARGS__)
#define FT_LOG_WARNING(...) FT_LOG(ft::Logger::WARNING, __VA_ARGS__)
#define FT_LOG_ERROR(...) FT_LOG(ft::Logger::ERROR, __VA_ARGS__)
}  // namespace ft
