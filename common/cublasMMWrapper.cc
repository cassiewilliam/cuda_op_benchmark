                           ...
                           float             f_beta)
{
    ...
    // printf("using_cublasLt %d, %d\n", using_cublasLt, findAlgo);
    if (using_cublasLt) {
        ...
        else
        {
            ...
            cublasLtMatmulAlgoGetHeuristic(cublaslt_handle_, operationDesc, Adesc, Bdesc, Cdesc, Cdesc, preference, 1, &heuristicResult, &returnedResults);

            int32_t algo_id;
            size_t returnSize;
            cublasLtMatmulAlgoConfigGetAttribute(&heuristicResult.algo, CUBLASLT_ALGO_CONFIG_ID, &algo_id, sizeof(algo_id), &returnSize);
        }
        ...
        cublasLtMatmul(cublaslt_handle_,
                       ...
                       C,
                       Cdesc,
                       (findAlgo == 1 ? (&algo) : (&(heuristicResult.algo))),
                       workSpace,
                       workspaceSize,
                       stream_);
...
void cublasMMWrapper::GemmX(cublasOperation_t  transa,
                            cublasOperation_t  transb,
                            const int          m,
                            const int          n,
                            const int          k,
                            ...
                            float              f_beta)
{
    ...
    if (using_cublasLt) {
        ...
        // reference from https://blog.csdn.net/eloudy/article/details/125827388
        cublasLtMatmulDescSetAttribute(operationDesc, CUBLASLT_MATMUL_DESC_EPILOGUE, &epilogue, sizeof(cublasLtEpilogue_t));
